...
public:
  ...
  float fit(const LinePair& hLinePair, const LinePair& vLinePair, cv::Mat binaryImage,
    cv::Mat rgbImage);
...
private:
  ...
  float evaluateModel(const std::vector<cv::Point2f>& courtPoints, cv::Mat binaryImage);
