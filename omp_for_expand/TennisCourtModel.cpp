...
{
  transformationMatrix = o.transformationMatrix.clone();
  bestScore = o.bestScore;
  return *this;
}
  ...
  if (points.size() >= 4)
  {
      for (auto& modelHLinePair : hLinePairs)
      {
          for (auto& modelVLinePair : vLinePairs)
          {
              ...
              Mat matrix = getPerspectiveTransform(modelPoints, points).clone();
...
float TennisCourtModel::evaluateModel(const std::vector<cv::Point2f>& courtPoints, cv::Mat binaryImage)
{
  float score = 0;
